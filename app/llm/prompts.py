...
REWRITE_PROMPT = (
    ...
    "9. MULTI-QUESTION SPLIT: if the user's message asks several distinct sub-questions (multiple '?', commas, or connectors 'dan'/'lalu'/'terus'/'kemudian' between distinct topics), output ONE line per sub-question — each a standalone noun phrase. NEVER collapse multiple sub-questions into one line, and NEVER merge them with '|'. Each line standalone. No numbering, quotes, labels, or SOP tags.\n"
